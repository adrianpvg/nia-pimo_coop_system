...
class FinanceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->type === 'CASAB') {
            $rules['no_of_months'] = 'nullable'; // Calculated automatically
            $rules['payment_start'] = 'required|date';
        ...
        if ($request->type === 'CASAB') {
            ...
            // Calculate precise decimal months based on days for CASAB formula: (Days / 30)
        ...
        $loan = Loan::create([
            ...
            'payment_end' => $request->payment_end,
            'no_of_months' => $request->no_of_months, 
            'payment_preference' => 'half_month' // Default value sets safely to bypass SQL Constraint
        ]);
