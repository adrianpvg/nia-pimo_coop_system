                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-2 mt-3">
                                        ...
                                        <div class="input-group glass-input" style="padding: 0; overflow: hidden; border-color: rgba(0,0,0,0.1);">
                                            <input type="number" step="0.01" id="interest_rate_input" class="form-control bg-transparent border-0 py-2 px-2 shadow-none text-center fw-bold" value="1.5" min="0" oninput="calculateAll()" required>
...
<script>
    ...
    function printLoanSchedule() {
        ...
        if (amount <= 0 || months <= 0) {
            alert("Please enter a valid Principal Amount and Duration before printing.");
            return;
        }

        let dateGranted = document.getElementById('date_applied').value || new Date().toISOString().split('T')[0];
        ...
        let currentDate = new Date(startDateInput || new Date());
        ...
        let rowsHtml = `
            <tr style="font-weight: bold; background-color: #f8f9fa;">
                <td></td>
                <td style="text-align: left; padding-left: 15px;">Principal</td>
                <td></td><td></td><td></td>
        ...
        for (let i = 1; i <= totalPeriods; i++) {
            ...
            let monthName = monthNames[periodStart.getMonth()];
            let startDay = String(periodStart.getDate()).padStart(2, '0');
            let endDay = String(periodEnd.getDate()).padStart(2, '0');
            let periodStr = monthName + ' ' + startDay + '-' + endDay + ', ' + periodStart.getFullYear();

            let principalDue = (i === totalPeriods) ? runningBalance : basePrincipalDue;
            let interestDue = monthlyBalance * halfMonthRate;
            let totalDue = principalDue + interestDue;

            runningBalance -= principalDue;

            sumPrincipal += principalDue;
            sumInterest += interestDue;
            sumTotal += totalDue;

            let balanceDisplay = !isFirstHalf ? '₱ ' + Math.max(0, runningBalance).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) : '';

            rowsHtml += `
                <tr>
                    <td>` + i + `.0</td>
                    <td style="text-align: left; padding-left: 15px;">` + periodStr + `</td>
                    <td>` + principalDue.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + `</td>
                    <td>` + interestDue.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + `</td>
                    <td>` + totalDue.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + `</td>
                    <td style="font-weight: bold;">` + balanceDisplay + `</td>
                </tr>
            `;
        ...
        let printContent = `
            <html>
            <head>
                <title>Computation Sheet - ` + applicantName + `</title>
                <style>
                    ...
                    .sched-table { width: 100%; border-collapse: collapse; font-size: 13px; text-align: center; }
                    .sched-table th, .sched-table td { border: 1px solid #ddd; padding: 8px; }
                    .sched-table th { background-color: #f0f0f0; }
                    .totals { font-weight: bold; background-color: #f8f9fa; }
                    .signatures { margin-top: 50px; width: 100%; text-align: center; font-size: 14px; }
                    .signatures td { padding-top: 30px; width: 33%; }
                    .line { border-top: 1px solid #000; width: 80%; margin: 0 auto; margin-top: 40px; }
                </style>
            </head>
            <body>
                <div class="header">
                    <h3>NIA REGION 1 MULTIPURPOSE COOPERATIVE</h3>
                    <p>BAYAOAS, URDANETA CITY, PANGASINAN</p>
                    <h4 style="margin-top: 15px;">COMPUTATION SHEET (DRAFT)</h4>
                </div>

                <table class="info-table">
                    <tr>
                        <td width="20%"><strong>NAME OF APPLICANT:</strong></td>
                        <td width="40%" style="border-bottom: 1px solid #000;">` + applicantName + `</td>
                        <td width="20%"><strong>AMOUNT OF LOAN:</strong></td>
                        <td width="20%" style="border-bottom: 1px solid #000;">₱ ` + amount.toLocaleString('en-US', {minimumFractionDigits: 2}) + `</td>
                    </tr>
                    <tr>
                        <td><strong>DATE GRANTED:</strong></td>
                        <td style="border-bottom: 1px solid #000;">` + dateGranted + `</td>
                        <td><strong>TERM:</strong></td>
                        <td style="border-bottom: 1px solid #000;">` + months + ` mos</td>
                    </tr>
                    <tr>
                        <td><strong>PAYING PERIOD:</strong></td>
                        <td colspan="3" style="border-bottom: 1px solid #000;">` + startDateInput + ` to ` + endDateInput + `</td>
                    </tr>
                </table>
                ...
                <table class="sched-table">
                    ...
                    <tbody>
                        ...
                        <tr class="totals">
                            ...
                            <td>₱ ` + sumPrincipal.toLocaleString('en-US', {minimumFractionDigits: 2}) + `</td>
                            <td>₱ ` + sumInterest.toLocaleString('en-US', {minimumFractionDigits: 2}) + `</td>
                            <td>₱ ` + sumTotal.toLocaleString('en-US', {minimumFractionDigits: 2}) + `</td>
                            <td></td>
                        </tr>
                    </tbody>
                </table>
                ...
                <table class="signatures">
                    <tr>
                        <td><div class="line"></div><p>Prepared By</p></td>
                        <td><div class="line"></div><p>Approved By</p></td>
                        <td><div class="line"></div><p>Conforme (Borrower)</p></td>
                    </tr>
                </table>
            </body>
            </html>
